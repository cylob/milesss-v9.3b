...
typedef int prtf( char * buf, char const * format, va_list ptr );
typedef void funcptr(void);
funcptr * __stdcall GetProcAddress( void * hModule, char const * lpProcName );
void * __stdcall LoadLibraryA( char const * name );
...
int sprintf( char * o, char const * fmt, ... )
{
  ...
  {
    tried=1;
    p = (prtf*)GetProcAddress(LoadLibraryA("msvcrt.dll"),"vsprintf");
  }
  if (p)
  {
    i = p( o, fmt,(va_list) (&fmt+1) );
  }
  else
  {
    i = wvsprintfA( o, fmt, (va_list) (&fmt+1) );
  }
  return i;
}
...
int printf( char const * fmt, ... )
{
  char buf[ 1024 ];
  unsigned int i, o;
  i = sprintf( buf, fmt, (va_list) (&fmt+1) );
  for( o = 0 ; o < i ; o++ )
    if ( buf[ o ] == '\n' )
    {
